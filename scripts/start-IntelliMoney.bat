...
if !ERRORLEVEL! equ 0 (
    for /F "tokens=5" %%a in ('netstat -ano ^| findstr ":%_port% "') do (
        ...
        echo %WARN% Port %_port% in use by PID %%a !_proc! — %_name%
    )
    exit /b 1
)
exit /b 0
...
set /a "_count+=1"
curl.exe -s -o nul -w "%%{http_code}" "%_url%" | findstr "200 302 301" >nul
if !ERRORLEVEL! equ 0 (
...
if %ERRORLEVEL% neq 0 (
    echo %WARN% Docker CLI not found. Install Docker Desktop:
...
if defined CFG_FILE if exist "%CFG_FILE%" (
...
if "%DOCKER_MODE%"=="1" (
    echo %INFO% Backend is running in Docker. Waiting for health check...
    call :wait_for_url "http://localhost:8080/api/health" "Backend API on Docker" 45
    set "BACKEND_READY=%ERRORLEVEL%"
    set "DOCKER_BACKEND=1"
)
...
if "%DOCKER_BACKEND%"=="0" (
    ...
    if %ERRORLEVEL% neq 0 (
        echo %WARN% Index creation had issues — usually harmless; indexes auto-create on app startup.
...
set "DOCKER_FRONTEND=0"
if "%DOCKER_MODE%"=="1" (
    echo %INFO% Frontend is running in Docker nginx on port 3002. Waiting for health check...
    call :wait_for_url "http://localhost:3002" "Frontend on Docker" 45
    set "FRONTEND_READY=%ERRORLEVEL%"
    set "DOCKER_FRONTEND=1"
)

if "%DOCKER_FRONTEND%"=="0" (
    cd /d "%FRONTEND_DIR%"
...
:: --- MongoDB check ---
curl.exe -s "http://localhost:8080/api/health" 2>nul | findstr "ok" >nul
if %ERRORLEVEL% equ 0 (
    set "MONGO_OK=1"
) else (
...
:: --- WebSocket ---
curl.exe -s -o nul -w "%%{http_code}" "http://localhost:8080/api/v1/ws" 2>nul | findstr "426 400" >nul
if %ERRORLEVEL% equ 0 (
...
if "%FRONTEND_READY%"=="0" (
    ...
    call :log "Browser opened to http://localhost:%FRONTEND_PORT%"
) else (
    if "%BACKEND_READY%"=="0" (
...
echo   Login:        %CYAN%http://localhost:%FRONTEND_PORT%/login%RESET%
echo.
