...
export default function AuthGate({ children, requireOnboarding = true }) {
  const { user, loading, clerkConfigured, isSignedIn, authError, logout, refreshUser } = useAuth();

  if (loading) return <div className="centered">Loading IntelliMoney...</div>;

  if (!isSignedIn) {
    if (!clerkConfigured) {
      if (requireOnboarding) return <Navigate to="/connect-bank" replace />;
  ...
  if (!user) {
    return (
      <div className="centered" style={{ textAlign: "center", padding: "3rem 1rem", maxWidth: 420 }}>
        ...
        {authError && (
          <p style={{ color: "#f87171", fontSize: "0.85rem", marginBottom: "1rem" }}>{authError}</p>
        )}
